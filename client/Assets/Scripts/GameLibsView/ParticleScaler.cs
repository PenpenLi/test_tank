...
namespace TKGameView
{
    ...
    public class ParticleScaler : MonoBehaviour
    {
        ...
        void Update()
        {
            Quaternion targetRotation = Quaternion.Euler(0.0f, wind_angle, 0.0f);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * 3); //平滑地转向
        ...
        void Control_Wind_is_live(bool is_live)
        {
            gameObject.SetActive(is_live);

        }
        void Set_Wind(float y)
        {
            //风向
            // Debug.Log("BBBBBB      " + "   " + y);
            wind_angle = y;
        ...
        void Control_Wind(int method, float y, bool is_live)
        {
            if (method == 1)
            {
                Control_Wind_is_live(is_live);
                Set_Wind(y);
            }
            else if (method == 2)
            {
